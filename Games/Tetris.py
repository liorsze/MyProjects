...
def valid_space(shape, grid):
    acc_pos = [[(j, i) for j in range(10) if grid[i][j] == (0, 0, 0)] for i in
               range(20)]  # get all possible location only if it empty
    # flat list
...
def clear_rows(grid, locked):
    ...
    if inc > 0:
        for key in sorted(list(locked), key=lambda x: x[1])[::-1]:  # sort pos that has the same y value
            x, y = key
            if y < ind:
                newkey = (x, y + inc)
...
def draw_window(surface, grid, score=0, last_score=0):
    ...
    pygame.font.init()  # set font
    font = pygame.font.SysFont("comicsans", 60)
    label = font.render("Tetris", 1, (255, 0, 255))

    surface.blit(label, (top_left_x + play_width / 2 - (label.get_width() / 2),
                         30))  # print in the middle of the screen
